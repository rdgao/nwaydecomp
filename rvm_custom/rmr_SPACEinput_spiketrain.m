...
for iepoch = 1:nepoch
  ...
  for ifreq = 1:nfreq
    ...
    if any(L<-tol)
      error(['some eigenvalues were negative: min eig: ' num2str(L(find(L==min(L),1))) ', max eig: ' num2str(L(find(L==max(L),1))) ', tol: ' num2str(tol)])
    end
    if  any(~isreal(L))
      error('some eigenvalues were complex')
    end 
